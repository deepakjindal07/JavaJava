...
public class QuickSortMultiThreaded<T extends Comparable<T>> implements Runnable {
	...
	private void quickSort(T arr[]) {
		Thread t = new Thread(new QuickSortMultiThreaded<>(arr, 0, arr.length - 1));
		t.start();
		try {
			t.join();
	...
	public void run() {
		if (start < end) {
			...
			Thread t1 = new Thread(new QuickSortMultiThreaded<>(arr, start, partition - 1));
			Thread t2 = new Thread(new QuickSortMultiThreaded<>(arr, partition + 1, end));
